...
public class History {
	...
	// Checks if client exists in list, else a new client is created and inserted
	// into list
	...
	public class Client {
		private HashMap<Integer, Marshall> messageIdToReplyMap;
		...
		public Marshall searchForDuplicateRequest(int messageId) {
			Marshall reply = this.messageIdToReplyMap.get(messageId);
			if (reply != null) {
				ConsoleLogger.debug("Request already serviced. Resending reply");
			}
			return reply;
		}

		// After request is serviced, adds messageID and reply to hashmap
		public void addServicedReqToMap(int messageId, Marshall replyToServicedReq) {
